package mihael.pietri.nasa.framework
...
import mihael.pietri.nasa.HostActivity
import mihael.pietri.nasa.NASA_PROVIDER_CONTENT_URI
...
fun Context.isOnline():Boolean{
    ...
    connectivityManager?.activeNetwork?.let { network ->
        connectivityManager.getNetworkCapabilities(network)?.let { cap ->
            return cap.hasTransport(NetworkCapabilities.TRANSPORT_CELLULAR) || cap.hasTransport(NetworkCapabilities.TRANSPORT_WIFI)
        }
